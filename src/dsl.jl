...
function parse_variables(expr)
    @capture(expr, (vars__,)) && return Symbol[v for v in vars if v isa Symbol]
    return expr isa Symbol ? Symbol[expr] : Symbol[expr]
end

function parse_assignment(assignment)
    @capture(assignment, lhs_ = rhs_) || error("Expected assignment expression")
    outputs = parse_variables(lhs)

    if @capture(rhs, func_(args__))
...
macro fk(expr)
    ...
    return esc(quote
        function $name($(args...))
            ...
            assignments = extract_assignments(substituted_body)
            steps = [eval(parse_assignment(assignment)) for assignment in assignments]
            return FKModel((steps...,))
        end
    end)
end